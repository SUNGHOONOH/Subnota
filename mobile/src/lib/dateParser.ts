...
const TIME_PERIOD_ONLY_REGEX = /^\s*(아침|점심|저녁|낮|밤|새벽)(?=\s|$)/;

const KO_DATE_PARSE_REGEXES = [
  /(\d{4})년\s*(\d{1,2})월\s*(\d{1,2})일/g,
  /오늘|내일(?!\s*모레)|모레|어제|그제/g,
  /\d+일\s*(전|후)/g,
  /\d+주\s*(전|후)/g,
  /((?:다)+음|이번|저번|지난)\s*주\s*(월|화|수|목|금|토|일)요일/g,
];
...
const buildWeekendDate = (prefix: string | undefined, baseDate: Date) => {
  const today = startOfDay(baseDate);
  const currentDay = today.getDay();
  // Saturday
...
const defaultHourForPeriod = (period: string) => {
  if (period === '아침') return 9;
  if (period === '점심' || period === '낮') return 12;
  if (period === '저녁') return 18;
  if (period === '밤') return 21;
  if (period === '새벽') return 6;
  return null;
};
...
const withParsedTime = (
  ...
  if (!timeMatch || !timeMatch[0].trim()) {
    const periodMatch = TIME_PERIOD_ONLY_REGEX.exec(remainingText);
    if (periodMatch) {
      const hour = defaultHourForPeriod(periodMatch[1]);
      if (hour === null) {
      ...
      return {
        date: scheduledDate,
        length: matchLength + periodMatch[0].trimEnd().length,
      };
    }
    return { date, length: matchLength };
  }
  ...
  return {
    date: scheduledDate,
    length: matchLength + timeMatch[0].trimEnd().length,
  };
};

const parseWithKoDateParse = (
  text: string,
  baseDate: Date,
): DateMatch[] => {
  ...
  for (const regex of KO_DATE_PARSE_REGEXES) {
    regex.lastIndex = 0;
    let m: RegExpExecArray | null;
    while ((m = regex.exec(text)) !== null) {
      const parsed = parseKoreanDate(m[0]);
      if (!parsed) {
        continue;
      }
...
export const parseDates = (
  ...
  const matches: DateMatch[] = parseWithKoDateParse(text, baseDate);
  ...
  while ((m = RELATIVE_DATE_REGEX.exec(text)) !== null) {
    const delta = RELATIVE_DAYS[m[0]] ?? RELATIVE_DAYS[m[0].replace(/\s+/g, '')];
    if (delta === undefined) {
      continue;
    }
