...
const CalendarScreen = ({ platformConfig }: { platformConfig: CalendarPlatformConfig }) => {
  const { width } = useWindowDimensions();
  const [contentWidth, setContentWidth] = useState(0);
  const layoutWidth = contentWidth || width;
  const isMac = platformConfig.isMac;
  const isPhone = !isMac && layoutWidth < 760;
  const usesCompactWeekBoard = isPhone || (isMac && layoutWidth < 980);
  const usesCompactMonthGrid = isPhone || (isMac && layoutWidth < 980);
  const boardWidth = Math.max(1, layoutWidth - 20);
  ...
  const handleMoveBrick = useCallback(
    (id: string, nextDay: number, requestedOrder: number) => {
      ...
      const targetOrder = usesCompactWeekBoard
        ? Math.min(Math.max(requestedOrder, 0), MOBILE_SLOT_TIMES.length - 1)
        : requestedOrder;
      const sourceDay = moving.day;
      const sourceOrder = usesCompactWeekBoard
        ? getMobileSlotIndex(moving)
        : moving.order;
      const targetBrick = usesCompactWeekBoard
      ...
      const moveSingleBrick = (
        ...
        const nextTime = usesCompactWeekBoard
    ...
    >
      ...
      {mode === 'month' ? (
        <MonthGrid
          ...
          days={DAYS}
          isCompact={usesCompactMonthGrid}
          memos={memos}
          monthDays={monthDays}
        ...
        <WeekBoard
          ...
          isCompact={usesCompactWeekBoard}
...
const styles = StyleSheet.create({
  ...
  yearPanel: {
    ...
    padding: 18,
    width: '100%',
    maxWidth: Platform.OS === 'macos' ? 420 : undefined,
  },
  yearTitle: {
    color: '#2C2520',
    fontSize: 18,
