...
import {
  createInboxClientId,
  enqueueLocalInboxSession,
} from './localInboxQueue';
import type { InboxSaveInput } from './localInboxQueue';
...
export const saveInboxSession = async (input: InboxSaveInput) => {
  const clientId = input.clientId ?? createInboxClientId();
  try {
    return await createInboxSession({ ...input, clientId });
  } catch {
    return enqueueLocalInboxSession({ ...input, clientId });
  }
};
