...
const settingsStyles = read('features/settings/SettingsStyles.ts');
const settings = `${read('features/settings/SettingsModal.tsx')}
${settingsStyles}`;
const settingsLayout = settingsStyles.slice(
  settingsStyles.indexOf(
...
describe('크롬 hover는 중성 회색', () => {
  ...
  it('설정 행 동작이 pill로 감싸진다', () => {
    const link = settingsLayout.slice(
      settingsLayout.indexOf('.settings-reference-link {'),
    );
...
describe('알약 컨트롤', () => {
  ...
  it('칩과 키 캡이 알약이다', () => {
    ...
    const cap = settingsLayout.slice(
      settingsLayout.indexOf('.settings-reference-shortcut-value {'),
    );
  ...
  it('세그먼트는 테마 한 곳에서 알약이 된다', () => {
    ...
    const settingsSegmented = settingsLayout.slice(
      settingsLayout.indexOf(
...
describe('주변 메모 그래프 노드', () => {
  it('Topic 영역색과 구분되는 중립색을 쓴다', () => {
    expect(graph).toContain("export const GRAPH_NODE_COLOR = '#69717a';");
    expect(graph).toContain('activeEdge: GRAPH_NODE_COLOR,');
    expect(graph).toContain('defaultNode: GRAPH_NODE_COLOR,');
    expect(graph).not.toContain("export const GRAPH_NODE_COLOR = '#396f55';");
  });
